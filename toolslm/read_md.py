...
For local files, use `create_heading_dict_file(path)` (`~` ok) rather than reading the text yourself: sections are then directly editable. Each node records `start_line` (the heading's 1-based line number in the full document), `view(nums=True)` shows the section with document-absolute line numbers, and `view(lnhashs=True)` shows `lineno|hash|` addresses in exhash's CRC-32 format - valid targets for `file_exhash` commands against that file. Plain `view()` is just `.text`. A parsed tree persists while the file may change, so after any edit call `refresh()` on the root for a fresh tree before further edits; a stale address fails loudly at exhash's hash check rather than editing the wrong line.
'''
