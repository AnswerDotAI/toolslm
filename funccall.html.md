...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L83"
...
``` python
def union_schema(
    t, defs:NoneType=None
):
```

*Schema for union type `t`: its schema-representable members, unwrapped
if only one, `anyOf` otherwise*

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L50"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Not every Python type has a JSON Schema representation: bare `object`
and `Any` say nothing a model could act on, and a missing annotation is
treated the same way;
[`handle_type`](https://AnswerDotAI.github.io/toolslm/funccall.html#handle_type)
raises `TypeError` for them all.
[`union_schema`](https://AnswerDotAI.github.io/toolslm/funccall.html#union_schema)
builds a union’s schema from just its schema-representable members,
unwrapping when only one remains, so an annotation like `str|object`
(meaning “pass a string; other objects accepted at runtime”) collapses
to a plain string schema:
...
Fixed-length tuples (e.g. `tuple[int, str]`) are mapped to a JSON Schema
array with `prefixItems` for per-position types, which is accepted by
Anthropic. OpenAI/Gemini require `items` so this is added, along with
`minItems`/`maxItems` to enforce the fixed length. If all positions
share the same type, `items` is that type directly; otherwise it’s an
`anyOf` of the unique types.
...
Tool parameters must be fully annotated: a missing annotation, or a type
with no schema representation (bare `object` or `Any`), raises rather
than producing a junk schema. Unions are filtered to just their
schema-representable members, so `str|object` means “a string, but any
object accepted at runtime”, and collapses to a plain string schema. A
union with no representable member raises:
...
We treat non-specialised arrays as arrays of strs, since some LLMs
require a subtype:
...
[`minipy`](https://AnswerDotAI.github.io/toolslm/funccall.html#minipy)
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L181"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L198"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L220"
target="_blank" style="float:right; font-size:smaller">source</a>
...
*Coerce inputs based on function type annotations, repairing common LLM
container mistakes*
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L239"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L244"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
def get_schema_nm(
    nm:str, ns, dot2dash:bool=False, **kwargs
):
```
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L272"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Sometimes LLMs accidentally use a str instead of a real dict/list, or
pass `{}` instead of a list. We coerce these:

``` python
def list_test(a:list[str], b:list[str]=None, c:str|list[str]='x'): return a,b,c
test_eq(call_func('list_test', {'a': '["x","y"]', 'b': None, 'c': 'z'}, ns=[list_test]), (['x','y'], None, 'z'))
test_eq(call_func('list_test', {'a': {}, 'b': 'foo'}, ns=[list_test]), ([], ['foo'], 'x'))
```
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L284"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
async def list_test(a:list, b:list[str]=None, c:str|list[str]='x'): return a,b,c
test_eq(await call_func_async('list_test', {'a': '["x","y"]', 'b': None, 'c': 'z'}, ns=[list_test]), (['x','y'], None, 'z'))
```
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L299"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L311"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/toolslm/blob/main/toolslm/funccall.py#L319"
target="_blank" style="float:right; font-size:smaller">source</a>
